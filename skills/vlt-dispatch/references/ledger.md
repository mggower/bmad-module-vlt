...
- **Findings** — a *shape-annotated* pointer that fails its shape's key requirement — **excluding the two legacy lanes below, which are exempt from the key check** (*Backward compatibility*, `references/relay.md`): an `ask`/`answer`/`deliver` with no `ref`, or an annotated `handoff` with no path **on disk** (an **existence** test — the counting rules below draw the distinction). The finding's legal response: **the publishing partner re-fires the relay correctly keyed (an unsolicited inline delivery mis-fired as an annotated `handoff` re-fires as `deliver` with a publisher-chosen `ref`); the recipient checks the malformed line off as superseded** (its own tagged line — two-writer discipline holds).
- **The legacy lines** — two denominated lanes, reported as counts, **never a key-requirement finding**: a lane pointer stays denominated and key-exempt, and remains eligible to be a finding for any unrelated reason (*Backward compatibility*, `references/relay.md`); zero renders as the denominated zero, matching the wires idiom below. Un-annotated **pathless** pointers are legacy pre-shape traffic (exempt from the key check by design; see `references/relay.md`, *Backward compatibility*): "N legacy unkeyed pointers (pre-shape)". Shape-annotated **pathless** pre-era pointers failing their key requirement are proto-`deliver` traffic (lane membership and the era datum live at the same home): "N proto-`deliver` pointers (pre-shape)". Counts are **live-record** counts: a `vlt-decay` drain moves closed legacy blocks to the archive and the denominator re-baselines — reproducibility means two readers of the same live record agree, not that the number never moves.

  **Counting rules (what makes the denominators reproducible).** The **unit is the pointer line** — the check's own scope ("for every pointer in a `relay:` block") — never the block: a block's item count neither inflates nor deflates a denominator. A pointer line is **pathless** iff it carries no **key-path** — a trailing `→ [[…]]` link (or plain path) whose **written target sits under** the handoff zone (`_agent/handoffs/` or `_agent/specs/`). *Pathless* is a **location** test — the written target's location decides, **no disk read**; "no path **on disk**" (Findings, above) is an **existence** test — the written key-path's target is missing from disk, resolved under **wikilink semantics, extension-optional** (`path` or `path.md`). Two tests, both real, each said once. **Any other wikilink — research notes, wiki concepts — is payload and never counts as a path.** Lane membership per line: a path-keyed pointer is ordinary traffic; an un-annotated pathless line is the legacy lane; a shape-annotated pathless pre-era key-failing line is the proto-`deliver` lane (the era datum and the key-failing membership clause live at *Backward compatibility*, `references/relay.md`); every other key-requirement failure is a finding. Shape detection reads the **block header and the pointer-leading annotation only** — a shape annotation quoted in body prose is payload.
...
- **The pointer-integrity line agrees with a fresh grep of `relay:` blocks** — every finding is a shape-annotated key failure, and the two denominated legacy counts match the record: **pathless and lane membership per the counting rules above** (unit = pointer line). The factory-side instrument `tools/dispatch-lane-check.py` (module source repo) is the derivation's fixture-backed check of record.
