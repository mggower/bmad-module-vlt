...
**Backward compatibility (no backfill).** An un-annotated header **with a path** reads as `handoff`. Two legacy lanes exist, and for **both** the exemption covers the **key requirement only**: a lane pointer is tolerated as written, drained normally, exempt from the key check, reported by `ledger` as a denominated count — and **remains eligible to be a finding for any unrelated reason**. An un-annotated header **without a path** is **legacy pre-shape traffic** ("N legacy unkeyed pointers (pre-shape)"; also exempt from the idempotency guarantee it never had). A shape-annotated **pathless** pointer written **before `deliver` existed** whose **shape's key requirement is unmet as written** — an annotated `handoff` with no key-path, an `ask`/`answer`/`deliver` with no `ref` — is **proto-`deliver` traffic**, the shape's own pre-history ("N proto-`deliver` pointers (pre-shape)"); a shape-annotated pre-era pointer **whose key is present** needs no exemption and is **ordinary traffic**, not lane traffic. The **era datum**: "before `deliver` existed" means **before the first `deliver` pointer exists in this record** — per-record, derivable at read time, no version knowledge required. In a record containing **no `deliver` pointer at all**, every shape-annotated pathless key-failing pointer is proto-`deliver` traffic — the feature demonstrably never ran in that record. Neither lane is ever a legal form to write anew. No existing record is ever edited to conform.
