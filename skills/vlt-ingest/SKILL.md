...
depends_on: ["frontmatter@14", "wiki-index@2", "wiki-consolidation@1", "wiki-supersession@2", "write-verification@5", "decision-log@4"]
---
...
- **The drain.** Record the declined name in this ingest's research note under **Open Questions** (Step 5 — as rendered, with its location), and make it recoverable **by address** (the address rule: `{conventions}/frontmatter.md`): closing it needs a source the vault doesn't have (the usual case — a roster, a credited transcript) → **relay** (shape `ask`) to the partner whose act closes it; the vault's own pages settle it → a `maintenance` backlog item; nobody can say what closes it → `knowledge-gap` to `{backlog}`. A filed item's `why` names **substitution suspected in a machine-transcribed source**; its `closes when` names the verifying act. (Kinds: the operating contract, *The backlog*.)
