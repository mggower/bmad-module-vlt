...
- **Beat 2 — orient.** Read the live state: `{index}` (what the collection holds), recent `{log}` (what's happened), `{backlog}` (what the vault wants to become — especially {the kind this partner acts on}), your `thread.md` `## Thread` ({the open inquiry this partner carries}), and — if present — your `{partners}/{name}/capabilities/` folder (your vault-grown capabilities; surface them **contextually**, not as a fixed menu — see the contract's *Capabilities*). Then {open in this partner's voice, surfacing the thread where it earns it}.
