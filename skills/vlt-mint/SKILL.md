...
**Horizontal vs vertical partners (archetype guidance).** Most partners are *horizontal* — **function** partners (librarian / researcher / creative) defined by a mode of work that spans domains. A *vertical (**domain**) partner* (e.g. a Language Tutor) is instead defined by a **subject area**. When the becoming conversation reveals a domain partner, expect that it (a) **names its domain self-awarely**, (b) **runs a domain verb** — most often the **longitudinal loop** (run a *program* over weeks: design → log → review/adjust), in which case it **wears the shared `vlt-track` hand** rather than minting a duplicate: add a `capabilities/track.md` heavy pointer (`procedure: { skill: vlt-track }`) carrying a *Loop profile* block (see `assets/capability-template.md`), which supplies the partner's `{root}`/`{target}`/streams/voice/non-negotiable. A genuinely-novel domain verb that **isn't** a longitudinal loop still mints its **own** operation skill. And (c) it **may need a bounded convention widening** — e.g. the personalized-extraction allowance in `{conventions}/extraction.md` that lets a domain deliverable cite the user's agent-zone state (the `vlt-track` loop *requires* this widening). Recognize the archetype *here* so the brief and the Phase-2 gate account for the loop-hand or op skill and any widening up front. (Keep this light; don't promote "vertical vs horizontal" to a contract concept until real domain partners accumulate.)
...
- **Contents:** the kind; the brief; architecture decisions + rationale; staged-artifact absolute paths; the **current phase + a done/pending checklist**; the council verdict (or "not yet run", or "not fielded — <why>; parked" per Step 2a's fallback); open user-decisions with their resolutions.
...
3. **Capture is mandatory, not optional.** Before the mint goes live, record the verdict **and its reasoning** in the mint decision log (`_agent/mint/decision-log.md`) for the mint, and annotate the originating `{backlog}` item with the outcome. A gated change must carry its own rationale. The verdict line carries its **provenance** per `{conventions}/decision-log.md` (*Verdict provenance*) — `council` with the workflow's `lensesFielded`, `council-degraded` with its note, or `user-ruled` with the required why; the vocabulary is single-homed there, not restated here. A verdict that **defers an enforcement leg with no convention frontmatter to carry the deferral** (an "until it bites" deferral) MUST register a wire in `{tripwires}` — `id`, `metric` (from the vitals reader's canonical table), `threshold`, `review_after` and the other schema fields per the registry's header — recorded in the same human-gated moment as the verdict itself. The registry is written **only** at these moments and by deliberate human edit; **no op ever writes counters** (the derive-first invariant, cited).
4. **Act on the verdict:** proceed on `pass`; apply the named changes and re-stage on `revise`; stop (and say why) on `reject`.

**When the council cannot be fielded (the named fallback — never improvise):** if the Workflow tool is unavailable in this environment (fan-out disabled, a constrained or headless runner), the gate does not dissolve and the mint does not self-review. Exactly two legal paths:

- **Park (the default — the only path in an unattended session).** Write the condition into the planning doc (`council verdict: not fielded — <why>; parked`), leave the checklist incomplete at Phase 2, and stop. The activation-time resume scan picks the mint up in a session that can field the panel — parking is the *designed* response, not a failure.
- **User-ruled verdict (attended sessions only, the user's explicit act).** The user — the vault's boundary-setter — may review the staged diff and rule `pass`/`revise`/`reject` themselves. Record it with `user-ruled` provenance per `{conventions}/decision-log.md` (*Verdict provenance*): the panel-not-fielded *why* is required. The minting context may never substitute its own synthesis for the panel — a self-review is not a fallback, and the panel's composition lives in the workflow, not here.

**Exit gate — Phase 2 → 3:** the council verdict is **resolved** (a `pass`, or a `revise` applied and re-staged to pass — fielded by the workflow, or user-ruled under the named fallback in Step 2a, recorded with its provenance) **and** every open user-decision is **ruled** — **and a boundary-creating mint (per the Phase-1 classifier) cannot pass with neither a bell nor a valid deferral.** Council-none kinds (a light or lane-rightful `add a capability`, `migrate a capability`, `retire a capability`, `create/extend a family`) clear this phase *trivially* — the gate predicate returns "no review required" — but still pass *through* it (the phase boundary holds; they just have nothing to validate). For a gated kind, write the verdict + decisions into the planning doc before building. A gated mint whose council cannot be fielded and whose user has not ruled it **parks** (planning doc, Step 2a's fallback) — parked is a legal resting state; an unrecorded substitute review is not.
...
**The spec consumer lock (binds every kind):** a mint or capability change that makes a partner consume an existing spec (a `{specs}` contract — see `{conventions}/spec.md`) edits that spec's `consumers:` in the **same change**, and `_agent/mint/decision-log.md` records it — consumer registration is never deferred to goodwill or memory.
...
- **Own-zone → light.** Instantiate the *light capability* template into `{partners}/{name}/capabilities/{slug}.md` (frontmatter object + the partner's application body). `weight: light`, `council_class: none` derive automatically. It writes **only the owning partner's own zone** — no shared lane. Register **nothing** in the help CSV (it is partner-zone state, surfaced on activation, not a globally-callable op). **Same verb, new source type → this light path:** the capability profile *is* the per-source-type front-end (the *source-type front-end* pattern — see `assets/capability-template.md`); no registry row — only the heavy weight registers a CSV row. Create the partner's `capabilities/` folder lazily. **Self-grow shortcut:** a partner may write this file *itself* mid-conversation without a full mint ceremony — the only ceremony is **one line to `_agent/mint/decision-log.md`** (keeps the evolution trail; the lane-safety of own-zone-only writes makes it safe).
...
3. **Read the convention's `consumers:` and walk every listed skill.** For each consumer whose text encodes the changed rule, make the matching edit and **bump its `depends_on` entry** for this convention to the new version (the flat `"name@version"` ack). Reconciliation may legitimately conclude "no edit needed here" — bumping the ack still records that a human verified it against the new version. Workflow assets that read conventions appear in the convention's `consumers:` by **filename** (e.g. `vlt-lint-full.js`) and are walked like any consumer — their ack is the flat `"name@version"` pin in the file's `// depends_on:` header line, bumped on reconciliation exactly as a skill's `depends_on:` is.
...
- **CSV field-quoting (always-quote) — whenever you write a `module-help.csv` row.** The free-text columns (`description`, `args`, `outputs`/`output-location`, `display-name`) routinely contain commas. Write each free-text field **wrapped in double quotes**, *always* — not "only when it currently has a comma" (always-quote is drift-proof: a later edit that adds a comma can't reintroduce the bug). An unquoted comma splits the row into too many columns and corrupts the registry. The merge reader now skips-and-reports such a row rather than registering it (so a mis-quoted mint silently fails to register — see `merge-help-csv.py`), which is exactly why the write side must not produce them. This holds for **every** live-registry row written below.
- **Provenance — every mint is vault-local.** `vlt-mint` runs *in an installed vault*, so everything it mints is vault-local: register **only** in the live registry `{project-root}/_bmad/module-help.csv`. **Never write the shipped install manifest** (`{module-skills}/vlt-setup/assets/module.yaml` `agents[]` or `.../module-help.csv`) — those are module-owned and refreshed on every upgrade, so a local write there is futile (clobbered on the next `vlt-upgrade`), records a false shipped-provenance fact, and — for a partner — writes the mint into the very `agents[]` that `vlt-upgrade`'s pre-flight reads to tell local from shipped, voiding its own B2 body-restore (see `vlt-upgrade`, Step 1). A local mint's durability is the merge's job — B1 preserves its live-registry row (`merge-help-csv.py`, keyed off the live skill dir still existing) and B2 restores its body — never a shipped-asset write. Registering into the shipped manifest is a factory act (a module release), out of scope for a field mint. The bullets below name only the **live-registry** writes; none touches `vlt-setup/assets/*`.
...
- A **heavy capability / operation skill** lives at `{module-skills}/vlt-{op}/` and becomes available to any partner; register its capability row in the live help registry `{project-root}/_bmad/module-help.csv`.
...
- A **capability migration** (heavy) updates the help registry attribution for the migrated op to the new owning partner; no new skill is created. A **light** migration moves the capability file between partner zones — nothing in the registry.
