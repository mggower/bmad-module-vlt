...
`<mode>` is `scoped since <timestamp>` or `full`. This entry is, **by derivation, the `lint-debt` counter reset** — no bookkeeping step exists anywhere. Also **persist the report** (both modes): write the Step-5 report block **verbatim** to `{lint_reports}/YYYY-MM-DD-HHMM-lint.yaml` — plain YAML, the block's content without the fence (append-only — never edit, prune, or re-read-to-rewrite past reports; retention is the human's — lint reports are never wake-read; the operating contract's *Decay contracts* table records the exemption). Pre-existing `.md` reports in `{lint_reports}` stay as they are — legal, never converted or swept. Write **no** session note — the summoning partner owns the session log (operating contract § session-ownership). A full-mode sweep also rewrites the findings cache at `_agent/lint-cache.yaml` — mechanics at `references/full-scale.md` (the fan-out protocol's single home). It is **not** a report: it is never persisted to `{lint_reports}`, never wake-read, and deleting it costs only a cold run.
