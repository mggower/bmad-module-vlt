...
depends_on: ["frontmatter@4", "wiki-index@2", "wiki-supersession@2", "extraction@3", "write-verification@1", "spec@1", "consult@1"]
---
...
flag_for_human:
  ...
  entity_scan: <P pages compared in Q clusters; single-mention substitutions are invisible by construction>   # denominator + blind spot — a bare zero below is not health
  entity_collisions: [<page-a vs page-b: <name> — <attribute A> vs <attribute B> (suspected source substitution)>, ...]
...
**Entity-collision reporting.** A conflict reported in `entity_collisions:` is **not** also reported in `contradictions:` — one finding, one slot (the precedence rule stated in Step 2). `entity_scan:` carries the population actually compared — pages, and clusters in full mode (from `files_checked` and the cluster cap the workflow surfaces) — **and** names the blind spot beside it: a substitution that entered once and was never contradicted cannot be seen by a cross-page check, and a cluster-bounded sweep did not compare every pair. **You compose that line yourself** in both modes, as with `contradiction_scan:`. Per the operating contract's honest-reporting rule — read it there.
