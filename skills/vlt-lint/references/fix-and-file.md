...
- **Index drift** — add missing pages (structural row + right category per `{conventions}/wiki-index.md`), remove non-existent ones, move resolved stubs out of the Stubs section (`{conventions}/wiki-index.md`); set the index `last_updated`. Do **not** add or "correct" source counts/dates — the index doesn't carry them.
...
- **Sources-vs-prose (`sources_vs_prose_mismatches` only)** — add the frontmatter `sources:` entries the page's prose `## Sources` section omits, in the section's existing form. **Add only; never remove a prose citation to make the two sides agree.** Each added entry is a **member of the existing `sources:` list** — never a new top-level frontmatter key, and never a bare list item outside `sources:`. If `sources:` is absent, create it as a list rather than writing loose items. **Re-parse the frontmatter after the edit**: a page whose frontmatter no longer parses is a defect this step caused, not a finding it cleared. The `flag_for_human` half (`sources_vs_prose_unresolved`) is **not** auto-fixed — the direction test and both legal responses live at `checks.md`, *Sources-vs-prose agreement*.
...
For each **spec candidate** (Step 2, the governance check), append its item too — the named owner and closes-when come from the beat's single home (`{conventions}/spec.md`, *Promotion from candidate*). Filing the item is also what makes the next run's report **quiet** for this candidate — the open item is the repeat record the Step-2 partition reads (`checks.md`, *Spec candidates* — the mechanics live there):

```
- [ ] Promote <handoff-doc> to {specs} (maintenance, by: <owning partner>) — spec_candidate: <signal, e.g. re-relay (2 same-key handoff entries)>; closes when: promoted per {conventions}/spec.md *Promotion from candidate*, or declined with reason recorded
```

**Guard:** an existing item for the same doc — open → mention it in-flow, never file a second (the duplicate-filing posture above); **and where the open item's recorded `spec_candidate: <signal>` clause differs from the signal this run just derived, update that clause in place to the current signal** — lint edits only its own clause on its own filed item, never the checkbox, the `by:` owner, or the closes-when (never-auto-promote untouched). That open item **is** the repeat record the Step-2 check partitions on (`checks.md`, *Spec candidates*) — a stale clause would read as "changed" every subsequent run and the collapse would never converge, so the refresh is mandatory, not cosmetic. Closed as **declined** → do not re-file (the recorded decline is honored; the count of honored declines is stated beside the finding in Step 5).
