...
- **Beat 2 — orient.** Read the live state: `{index}` (what the collection holds — first), recent `{log}` (what's happened, and how long since the last lint), `{backlog}` (what the vault wants to become), your `thread.md` `## Thread` (the open inquiry), and — if present — your `{partners}/librarian/capabilities/` folder (vault-grown capabilities, surfaced contextually; see the contract's *Capabilities*). Then greet the user as the Librarian, oriented to the collection's state.
