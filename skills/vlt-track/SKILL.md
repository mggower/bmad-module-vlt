...
depends_on: ["extraction@10", "wiki-supersession@2"]
---
...
- **The polished protocol lives in PARA** — the profile's *PARA target* holds the curated, human-facing deliverable, written via **personalized-extraction discipline** (see `{conventions}/extraction.md`, *Personalized extraction*). Its `sources:` cite **only the wiki pages** that supply the method (the hard, load-bearing invariant — every method claim traces to a wiki page), and a **separate** `personalization_sources:` field cites the agent-zone log(s) that supply the subject's lived state (situation, not fact). This op's PARA deliverable is an **extracted artifact** and reaches the layer by that disposition alone (`{conventions}/extraction.md`; the layer's entry condition lives in the operating contract, Layer 3) — the widening here is in what an extraction may cite *for personalization*, nothing more.
...
`vlt-track` is the **module-shipped** op **named in the base** of `extraction.md`'s *Personalized extraction* — so the shared hand's own use of the widening is sanctioned by the base, and running the loop it ships to run needs no further grant. What a **calling domain partner** still earns through its **own gated mint** is any *extension of that allowance for its own landing zone* — a vertical partner writing to `areas/<domain>/…` from its own agent-zone streams names that per-op extension in the vault's `extraction.overlay.md` (see `vlt-mint`, *Edit a convention*). If a partner is invoking a domain-specific personalization its mint never granted, **stop** — that extension is earned through a gated mint, not taken here. This gate is about a domain op's *own* extension; it is **not** a bar on `vlt-track`'s base-granted sanction.
