...
- For each of: `_meta/vault-operating-contract.md`, `_meta/conventions/{frontmatter,wiki-index,wiki-supersession,wiki-consolidation,extraction,spec}.md`, `_meta/personas/{architect,skeptic,pragmatist,historian,moderator}.md` — if the file already exists in the vault, **skip it** (the vault's own version wins) and note the skip; otherwise copy it in.
