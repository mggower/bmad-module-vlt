...
**Stash the skill-asset manifest (durability — build-18).** Conventions have the baseline above; the **shipped skill surface** needs the same divergence net so a local hand-edit of an installed skill asset isn't silently clobbered on upgrade. Once the shipped skills and workflows are in place, write a **SHA-256 manifest** to `{overlays}/.baseline/.skill-manifest` — one `<sha256>␉<path>` line per shipped file, covering every **shipped** `vlt-*` skill dir (those in `module.yaml` — i.e. **not** a locally-minted `vlt-agent-*`), across `SKILL.md` + everything under each skill's `assets/`, plus the installed `.claude/workflows/*.js`. Like the baseline (and the workflows), the manifest is **module-owned: overwrite it to the current shipped versions on every install/update**. It lets `vlt-upgrade` tell a local hand-edit of a shipped skill asset (live SHA ≠ manifest) from a clean install (live SHA == manifest) — exactly as the baseline does for conventions. Compute it from the *installed* shipped files (which equal stock at install time), so the next upgrade's pre-flight compares against what this install shipped.
...
- **Create `{lint_reports}`** (default `_agent/lint-reports/`, empty dir) if absent — `vlt-lint` persists its dated Step-5 report blocks there. Same never-clobber line: existing reports are the vault's, append-only, untouched.
