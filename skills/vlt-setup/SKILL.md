...
- **Create `{lint_reports}`** (default `_agent/lint-reports/`, empty dir) if absent — `vlt-lint` persists its dated Step-5 reports there (plain `.yaml` by default, or `.json` — both legal; the persist step's single home is `vlt-lint` Step 6). Same never-clobber line: existing reports are the vault's, append-only, untouched.
