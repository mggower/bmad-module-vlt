...
| `resources`   | `resources/`                              | **Unbounded containers and their artifacts (Layer 3, *PARA containers*) — an active domain that contains the wiki: the `{wiki}` subtree nested here is Layer-2 Librarian-only territory, not PARA** |
