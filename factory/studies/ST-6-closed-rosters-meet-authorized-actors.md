...
cited_by:
  - 'factory/cycles/15-nothing-reads-it-back/roadmap.md §A15-13 — **INSTANCE 3**, and the first with a SHIPPED contradiction rather than a drift: `checks.md:20` resolves `verified_by:` → partner slug, a value `write-verification.md:47` makes illegal, so `para_writer_unauthorized` ships a resolution leg no conformant vault can exercise. The enabling cause (no check that a grant and its roster cover the same population; visible only to a reader holding both files) is exactly how it survived. Capture relocated this filing''s primary study citation here from ST-2.'
