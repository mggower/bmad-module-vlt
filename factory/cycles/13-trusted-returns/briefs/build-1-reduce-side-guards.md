...
status: 'BUILT 2026-08-26 on branch `cycle13-v0.16.1` — one commit, all four F-sites landed, tag/ff-merge/push HELD for the owner per `vlt-release`.
  **F1 `vlt-lint-full.js`** — the `malformed_frontmatter` reduce (was `:609`, now `:688`) filters on `frontmatter_valid === false && !refusedFrontmatterClaim(s.frontmatter_issue)`; the guard block (`:549-620`) adds `PAGE_REQUIRED_FRONTMATTER` (11 names, read from the WIKI PAGE schema in `{conventions}/frontmatter.md` *Wiki pages*, NOT `PAGE_SCAN.required`), `PAGE_OPTIONAL_FRONTMATTER` (3), `ATTESTATION_FRONTMATTER` (2), `parseClaim()` and the two named predicates. +95/-2 lines.
  **F2 `vlt-lint-full.js`** — `unmarked_supersessions` (was `:576`, now `:651`) inlines collect''s body at the CALL SITE with `.filter((v) => !attestationOnlyComplaint(v))` applied to the RAW returned value before the `slug: ` prefix exists. `collect()` at `:542` is byte-identical to HEAD (`diff` clean) and its two other consumers (`stale`, `contradictions`) are byte-identical.
  **F3 `skills/vlt-lint/references/checks.md:15`** — the *Malformed frontmatter* check entry, beside *Frontmatter / Bases-field drift*: population, both exclusions with reasons and destinations, the conjunction rule, and the legal response the class had never had.
  **F4 `skills/vlt-lint/references/report.md:66`** — the `malformed_frontmatter:` slot with its exclusions inline.
  **Verification, as real output.**
  (V1, the harness — shipped source, stubbed runtime, `args` a JSON string) `malformed_frontmatter: ["nc-genuine-break: missing title", "nc-compound-break: missing title and missing `verified_by`/`verified_at`", "nc-compound-prose: missing `verified_by`; YAML block unterminated"]` · `unmarked_supersessions: ["nc-genuine-supersession: claim silently updated with no [!superseded] callout"]` · `unattested_write:` all 11 fixture pages incl. the six real slugs · `attestation_census: {pages_total: 11, fresh: 0, stale: 0, unattested_pre_adoption: 11}`. Five attestation-only entries + the one attestation `unmarked_supersession` + the invented-requirement entry REFUSED; four negative controls SURVIVE (genuine break, field-named compound, prose-half compound, genuine supersession).
  (V1b, predicate unit run, 16 cases) 16/16 PASS, incl. `missing source_type` → refused and `missing sources` → reported.
  (V1c, whole-return A/B vs HEAD on one fixture) exactly two keys differ — `fix_now.unmarked_supersessions` and `flag_for_human.malformed_frontmatter`; every other key byte-equal.
  (V2) `collect()` `:542` byte-identical; other consumers byte-identical.
  (V3) all six refused pages present in `unattested_write` and counted in `attestation_census.unattested_pre_adoption`.
  (V4) `grep -rn "malformed_frontmatter" skills/` → **3** hits (emitter `:688`, `checks.md:15`, `report.md:66`) where HEAD returned **1**; workflow `depends_on:` `:11` and `vlt-lint/SKILL.md:4` byte-identical.
  (V5) `uv run tools/package-lint.py --expect-version 0.16.1` → `package-lint: A/B/C/E PASS, D PASS — vlt 0.16.1`.
  (V6/V8) n/a as briefed. (V7) satisfied by F3. (V9) scrub clean — see deviation 3. (V10) no `.decision-log.md` in the tree.
  **Acceptance evidence.** (1) PASS — V1 above. (3) PASS — V4 + V5 above. (2) PASS on its literal terms but **NON-PROBATIVE as run, stated so rather than banked**: the six live haiku probes over read-only copies of the real pages ALL returned `frontmatter_valid: true`, empty `frontmatter_issue`, empty `unmarked_supersession` — they did not reproduce the misroute, so zero of the six reach either class, but the guard was never exercised and the identical returns through the HEAD reduce give a byte-identical result. Closed with the probative variant on the same real corpus, carrying the returns the FIELD produced on 2026-08-25: PRE-GUARD `malformed_frontmatter` 5 entries + `unmarked_supersessions` 1 entry → POST-GUARD **both empty**, `unattested_write` all six under their real `created` dates, `attestation_census: {pages_total: 6, fresh: 0, stale: 0, unattested_pre_adoption: 6}` unchanged across both. (4) field-contingent, owner-run, unchanged.
  **Deliberate deviations.** (1) F4''s slot went to `flag_for_human` after `thin_pages`, not to `fix_now` beside `frontmatter_drift` as the brief said — the emitter puts `malformed_frontmatter` in `flag_for_human`, so the brief''s placement would have documented a key the workflow never emits there. (2) Disposition 2 implemented as "every claimed-missing field is a DOCUMENTED-OPTIONAL page slot" rather than the brief''s literal "claimed field ∉ required-set" — the literal negation refuses any missing-claim naming an unrecognized token (e.g. "missing closing delimiter"), which is the over-broad guard acceptance check (2) exists to catch; the guard now refuses only what it can POSITIVELY identify, and a claim it cannot decide always reports. (3) The disposition-2 code comment initially named the live vault page slug that produced the defect; the scrub caught it and the comment was rewritten to describe the confusion without the slug. (4) A fourth negative control (a compound whose non-attestation half is free prose, not a schema field name) was added beyond the brief''s three — the brief''s three would not have caught the residue-rule gap. (5) The build-time memlog is homed at `factory/cycles/13-trusted-returns/build-1.memlog.md`, not under `skills/`: `.memlog.md` is not gitignored and `vlt-upgrade`''s own-the-apply is a filesystem copy, so a memlog under the shipped surface would ship into every vault.
  **Also landed this run:** disposition 6''s deferred retirement of `malformed_frontmatter` recorded as carry 3 in the roadmap''s §Carried forward, with the `:159`/`:168` defence-in-depth survival stated explicitly.'
module_code: 'vlt'
created: '2026-08-26'
derives_from:
