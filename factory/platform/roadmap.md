...
### P-22 — name the acceptance check that has a trigger but no cause — **queued**

*(Queued 2026-09-01 from `factory/inbox/2026-08-31-104502-resources-write-legality-check-has-no-cause-and-survived-three-runs.md`. Routed here, **not** to the cycle roadmap: the filing's only fix site is `.claude/skills/build-brief/references/brief-anatomy.md` plus the brief vocabulary around it — factory skill references that `vlt-upgrade` never copies into a vault, so no cycle build could deliver the change. ⚠ **The filing carried NO `channel: platform` marker.** Under [P-3]'s fork, an unmarked-but-plainly-factory-side filing is surfaced to the owner rather than re-routed silently; the owner ruled it here at Cycle 15's capture. The boundary was re-derived from the site, not taken from the filing's own "Factory-side signal" note.)*

**Brief-lite:**
- **Intent:** the brief vocabulary has two grades — `[ship-verifiable]` and `[field-contingent]` — and no word for a check whose event has a **trigger but no cause**. Cycle 12 build-3's check (7) (*a partner resolves a `{resources}`-write legality question from the rewritten bundle without escalating*) survived **three discharge runs unfired** while ordinary activity of exactly its kind ran repeatedly: since the 0.17.1 upgrade the field vault took **at least eight `{resources}` writes** — five wiki pages rewritten by the 2026-08-30 sweep's own fixer, three brief issues written into `resources/briefs/` on 2026-08-31 — across partner sessions spanning the misc, extract, lint and groom lanes, and **no session record contains a `{resources}`-write legality question**. The flow ran; the surface under test was never reached. The check is *reachable* in the literal sense (the owner can open a session and pose the question at any time) and that is precisely the defect: **a check dischargeable only by staging its own evidence is not observing the field, it is asking the field to pose for it.** Cycle 14 amended ruling D3 so a **bounded** check gates, where a bound is *at rest, at the release gate, or on the next ordinary upgrade* — and the load-bearing word is **ordinary**. This species has no bound of any of the three kinds and no ordinary occasion either, and that gap has no name. Distinguishable from a legitimate field-contingent check (*"the next mint exercises the consumer lock"*, *"the next full sweep reports N"*) because those name an event the owner **already performs on cadence for reasons of their own**; this one names an event that exists only to be the evidence. Cousin of **[P-20]** (the check adversary) — P-20 attacks a check's *claim*; this names a defect in a check's *causal footing*, and the two may want to merge rather than sit adjacent.
- **Sites:** `.claude/skills/build-brief/references/brief-anatomy.md:203-210` — the unbounded species is already discouraged here; the no-cause species is the adjacent case with no word. `.claude/skills/build-brief/SKILL.md` §9 (Acceptance) — where a check is written and where a brief-time prohibition would bind. `.claude/skills/acceptance-discharge/` — the rubric's **BLOCKED** grade exists for events with no *trigger*; this species has one and still cannot discharge, so the grade's boundary needs stating. `factory/cycles/12-proxy-claims/roadmap.md:2896-2902`, `:3082-3088` — the originating check, for the worked example.
- **Verification:** at rest. Re-read the four Cycle 12/13/14 checks that were released as standing watches and grade each against the new vocabulary — the species is real only if it partitions them non-trivially (some no-cause, some legitimately field-contingent). A brief-time prohibition is gradeable by writing a check of the forbidden shape and confirming the brief refuses it.
- **Out of scope:** any change to `[ship-verifiable]` / `[field-contingent]` meanings or to D3-as-amended; the gating question (a no-cause check does not gate today and this item does not propose that it should); re-grading Cycle 12 build-3 check (7) itself — that grade is **BLOCKED (unreachable)**, recorded on Cycle 14's ledger, `[field-contingent]`, does not gate, and stays as graded.
- **Done-when:** the brief vocabulary names the third species and states at brief time that such a check may not be written; `acceptance-discharge`'s rubric says how the new grade relates to BLOCKED; and the next brief written after the change is checked against it (a cold reader, not this item's author, applies the vocabulary to a real check without re-deriving it).
